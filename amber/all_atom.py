...
def atom14_to_atom37(atom14_data: jnp.ndarray,  # (N, 14, ...)
  ...
  atom37_data = utils_model.batched_gather(atom14_data,
...
def atom37_to_atom14(
  ...
  atom14_data = utils_model.batched_gather(atom37_data,
...
def atom37_to_frames(
  ...
  # shape (N, 8, 3)
  residx_rigidgroup_base_atom37_idx = utils_model.batched_gather(
      restype_rigidgroup_base_atom37_idx, aatype)

  # Gather the base atom positions for each rigid group.
  base_atom_pos = utils_model.batched_gather(
      all_atom_positions,
  ...
  # (N, 8)
  group_exists = utils_model.batched_gather(restype_rigidgroup_mask, aatype)

  # Compute a mask whether ground truth exists for the group
  gt_atoms_exist = utils_model.batched_gather(  # shape (N, 8, 3)
  ...
  residx_rigidgroup_is_ambiguous = utils_model.batched_gather(
      restype_rigidgroup_is_ambiguous, aatype)
  residx_rigidgroup_ambiguity_rot = utils_model.batched_gather(
...
def atom37_to_torsion_angles(
  ...
  chis_mask = utils_model.batched_gather(params=chi_angles_mask, indices=aatype,
                                   axis=0, batch_dims=0)
  ...
  chi_angle_atoms_mask = utils_model.batched_gather(
  ...
  chi_is_ambiguous = utils_model.batched_gather(
...
def torsion_angles_to_frames(
  ...
  m = utils_model.batched_gather(residue_constants.restype_rigid_group_default_frame,
                           aatype)
...
def frames_and_literature_positions_to_atom14_pos(
  ...
  residx_to_group_idx = utils_model.batched_gather(
  ...
  mask = utils_model.batched_gather(residue_constants.restype_atom14_mask, aatype)
...
def extreme_ca_ca_distance_violations(
  ...
  return utils_model.mask_mean(mask=mask, value=violations)
...
def get_alt_atom14(aatype, positions, mask):
  ...
  # shape (num_res, 14, 14)
  renaming_transform = utils_model.batched_gather(
